...
Navigate directly to a specific HTML document by adding `?startdoc=DocumentName` to the page URL. The parameter name is matched case-insensitively, so `?Startdoc=DocumentName` also works.

### Usage

```
https://yourtenant.sharepoint.com/sites/yoursite/SitePages/YourPage.aspx?startdoc=UserGuide
```
...
|---|---|---|
| 1 | `?startdoc=` URL parameter | On initial page load only, when `?startdoc=DocumentName` is in the URL |
