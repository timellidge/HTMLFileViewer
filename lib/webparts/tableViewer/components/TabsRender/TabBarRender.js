...
export default function TabBarRender({ fieldName, tabs, handleTabChange }) {
    return (React.createElement("div", { className: styles.tabBar }, Object.keys(tabs).map(tab => (React.createElement("button", { key: tab, onClick: () => handleTabChange(fieldName, tab), title: `Click to filter on ${fieldName}: ${tab}`, style: {
            marginRight: '8px',
