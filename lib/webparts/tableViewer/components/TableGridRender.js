...
const TableGridRender = ({ colJSON, items }) => {
    ...
    const renderNumber = (item, key, column) => (React.createElement("div", { className: styles.numberCell }, item[key].displayValue));
    // DEFAULT RENDER FUNCTION WITH LINES CLAMP
    const renderLink = (item, key, column) => (React.createElement("a", { href: item[key].rawValue, className: styles.tableDataContent }, item[key].displayValue));
