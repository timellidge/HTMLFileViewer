...
const TableGridRender = ({ colJSON, items }) => {
    ...
    const renderNumber = (item, key, column) => (React.createElement("div", { className: styles.numberCell },
        column.prefix && React.createElement("span", null, column.prefix),
        item[key].displayValue,
        column.suffix && React.createElement("span", null, column.suffix)));
    // REnder a link
    const renderLink = (item, key, column) => (React.createElement("a", { href: item[key].rawValue, className: styles.tableDataContent },
        column.prefix && React.createElement("span", null, column.prefix),
        item[key].displayValue,
        column.suffix && React.createElement("span", null, column.suffix)));
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: _GridStyle },
            ...
            sortedItems.map((item, itemIndex) => (React.createElement(React.Fragment, { key: itemIndex }, _sortedColumns.map(({ key, column }) => (column.width > "0" && (React.createElement("div", { key: `${itemIndex}-${key}`, className: `${styles.tableCell} ${column.class ? column.class : ''}`, "data-row": item.ID.rawValue, onMouseEnter: (event) => handleMouseEnter(event), onMouseLeave: (event) => handleMouseLeave(event) }, column.type === 'stack' ? (renderStack(item, column, colJSON)) : item[key] ? (column.type === 'person' ? renderPersonCard(item, key, column)
        ...
        React.createElement("button", { onClick: toggleSidePanel }, "Toggle Side Panel"),
        React.createElement(Panel, { isOpen: isSidePanelOpen, onDismiss: toggleSidePanel, closeButtonAriaLabel: "Close", headerText: "Side Panel", type: PanelType.large },
