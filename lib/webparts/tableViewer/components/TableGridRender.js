...
const TableGridRender = ({ colJSON, items }) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: _GridStyle },
            ...
            sortedItems.map((item, itemIndex) => (React.createElement(React.Fragment, { key: itemIndex }, _sortedColumns.map(({ key, column }) => (column.width > "0" && (React.createElement("div", { key: `${itemIndex}-${key}`, className: `${styles.tableCell} ${column.class ? column.class : ''}` }, item[key] ? (column.type === 'person' ? (React.createElement(PersonCard, { email: item[key].rawValue[0].email, name: item[key].rawValue[0].name, title: item[key].rawValue[0].title, format: column.format })) : column.type === 'stack' ? (React.createElement("div", null, column.fields.map((field, fieldIndex) => (item[field] ? (React.createElement("div", { className: `stack ${field}`, key: fieldIndex }, item[field].displayValue)) : (React.createElement("div", { className: `stack ${field}`, key: fieldIndex }, "\u00A0")))))) : column.type === 'html' ? (React.createElement("div", { dangerouslySetInnerHTML: { __html: item[key].displayValue } })) : (item[key].displayValue)) : (column.type === 'stack' ? (React.createElement("div", null, column.fields.map((field, fieldIndex) => (item[field] ? (React.createElement("div", { key: fieldIndex, className: `stack ${field}` }, item[field].displayValue)) : (React.createElement("div", { key: fieldIndex, className: `stack ${field}` }, "No Data")))))) : (React.createElement("span", null, "No Data")))))))))))));
};
