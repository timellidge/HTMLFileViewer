...
import { getItemsUsingRenderListDataAsStream, numberFormat, toProperCase, getListUrl } from '../../../helpers/Utilities';
...
const TableViewerContainer = (props) => {
    ...
    const { displayMode, title, updateProperty, showTitle, showFind, configured, onConfigure, JSONCode, webPartCSS, siteUrl, listId, viewXmlCode, hideErrorEmpty, emptyMessage, themeVariant, contentHeight, contextSiteUrl, contextUser, sidePadding, webPartTag, tabBehaviour } = props;
    //=================================================================================================================
    ...
    const [maxBarValues, setMaxBarValues] = useState({});
    ...
    const getListPath = React.useCallback(async () => {
        try {
            const listPath = await getListUrl(siteUrl, listId);
    ...
    useEffect(() => {
        ...
        if (!ColumnsJSON || typeof ColumnsJSON !== 'object' || Object.keys(ColumnsJSON).length === 0 ||
        ...
        const interestingFields = Object.keys(ColumnsJSON).filter((key) => ColumnsJSON[key].type !== 'stack' && ColumnsJSON[key].type !== 'edit');
    ...
    return (React.createElement("div", { id: webPartTag, className: _containerClass }, updatedItems.length === 0 && hideErrorEmpty ? (React.createElement("div", { id: "emptymessage" }, emptyMessage) // Render an empty div if we have no data and hideErrorEmpty is true
    ) : (!configured ? (React.createElement(React.Fragment, null,
        React.createElement("div", null,
            ...
            React.createElement("div", { className: styles.tabBar }, Object.keys(tabData).map((field) => (React.createElement(TabBarRender, { key: field, fieldName: field, tabs: tabData[field], handleTabChange: handleTabChange, tabBehaviour: tabBehaviour })))),
            React.createElement(TableGridRender, { listUrl: listPath, colJSON: ColumnsJSON, items: filteredItems, contentHeight: contentHeight, maxBarValues: maxBarValues, height: windowDimensions.height })),
